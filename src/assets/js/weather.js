	...
	setTimeout(function(){
	    ...
	    urls[1] = 'https://api.map.baidu.com/telematics/v3/weather?output=json&ak=6tYzTvGZSOpYB5Oc2YGGOKt8&callback=getTodayWeather&location=' + encodeURI(cityName);
	...
	document.getElementById('search').addEventListener('click', function (){
		...
		if (cityname != ''){
            ...
            var callArgs = "[\"" + cityname + "\",\""+ cityname +"\",\""+ cityname + "\"]";
            console.log(callArgs)
            serialNumber = nebPay.call(to,value,callFunction,callArgs,{
                listener: callBackFun(cityname)
            });

		}
	}, true);

	function callBackFun(cityname) {
        var cityname = document.getElementById('input-weather').value;
        var urls = [];
